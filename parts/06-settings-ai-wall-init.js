...
async function boot() {
  ...
  sb.auth.onAuthStateChange((ev, session) => { S.session = session; S.user = session?.user || null; if (ev === 'SIGNED_OUT') location.reload(); if (ev === 'PASSWORD_RECOVERY') { recovery = true; newPasswordScreen(); } });
...
function signInScreen(msg = '', keepEmail = '') {
  authScreen(`<h1>HomeBase</h1><p class="sub">Sign in to your household.</p>${msg ? `<div class="note" style="margin-bottom:12px">${esc(msg)}</div>` : ''}
    ...
    <div class="alt" style="margin-top:8px"><button id="forgot">Forgot password?</button></div>
  ...
  $('#forgot').onclick = async () => { const email = $('#em').value.trim(); if (!email) return signInScreen('Type your email first, then tap Forgot password.'); localStorage.setItem('hb_last_email', email); const { error } = await sb.auth.resetPasswordForEmail(email, { redirectTo: location.origin + location.pathname }); signInScreen(error ? error.message : 'Check your email for a reset link — it brings you back here to choose a new password.', email); };
  $('#demo2').onclick = () => { localStorage.setItem('hb_demo', '1'); location.href = location.pathname + '?demo=1'; };
}
// Reached from the password-reset email (Supabase puts type=recovery in the URL and signs the user in)
function newPasswordScreen(msg = '') {
  authScreen(`<h1>New password</h1><p class="sub">Choose a password for ${esc(S.user?.email || 'your account')}.</p>${msg ? `<div class="note" style="margin-bottom:12px">${esc(msg)}</div>` : ''}
    <div class="field"><label>New password (8+ characters)</label><input id="np1" type="password" autocomplete="new-password"></div>
    <div class="field"><label>Again</label><input id="np2" type="password" autocomplete="new-password"></div>
    <button class="btn btn-primary btn-lg btn-block" id="npgo">Save and continue</button>`);
  $('#npgo').onclick = async () => { const a = $('#np1').value, b = $('#np2').value; if (a.length < 8) return newPasswordScreen('Use at least 8 characters.'); if (a !== b) return newPasswordScreen('The two passwords don’t match.'); const { error } = await sb.auth.updateUser({ password: a }); if (error) return newPasswordScreen(error.message); history.replaceState(null, '', location.pathname); toast('Password saved'); await afterSignIn(); };
}
